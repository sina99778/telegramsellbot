...
class Messages:
    ...
    RENEWAL_ENTER_TIME = (
        "📅 مدت تمدید را به <b>روز</b> وارد کنید.\n"
        "<i>مثال: ۳۰ یا 30 برای یک ماه</i>"
    )
    RENEWAL_INVALID_VALUE = "❌ مقدار وارد شده معتبر نیست. لطفاً یک عدد بزرگ‌تر از صفر بفرستید."
    RENEWAL_INVOICE = (
        "🧾 <b>فاکتور تمدید</b>\n"
        "━━━━━━━━━━━━━━\n"
        "🔋 حجم اضافه: <b>{volume}</b> گیگابایت\n"
        "📅 زمان اضافه: <b>{time}</b> روز\n"
    ...
    CONFIG_CREATED = (
        ...
        "━━━━━━━━━━━━━━\n"
        "🚀 پلن: <code>{plan_name}</code>\n"
        "💾 حجم: <code>{volume_label}</code>\n"
        "👤 نام کانفیگ: <code>{client_email}</code>\n"
        "━━━━━━━━━━━━━━\n"
        "🔗 <b>لینک اشتراک (ساب‌لینک)</b>\n"
        "👆 روی متن زیر بزنید تا کپی شود:\n"
        "<code>{sub_link}</code>\n\n"
        "⏱ <i>زمان اکانت از لحظه‌ی تحویل محاسبه می‌شود.</i>\n"
...
class AdminMessages:
    ...
    ENTER_VOLUME = (
        "حجم پلن را بر حسب گیگابایت وارد کنید.\n"
        "• عدد ≥ ۱ → همان مقدار حجم\n"
        "• <code>۰</code> → نامحدود (بدون محدودیت حجم)"
    )
    VOLUME_GT_ZERO = "حجم نمی‌تواند منفی باشد. عدد ≥ ۱ یا ۰ (نامحدود) وارد کنید."
